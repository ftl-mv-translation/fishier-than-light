...
script.on_internal_event(Defines.InternalEvents.PRE_CREATE_CHOICEBOX, function(event)
    ...
    if event.eventName == "STORAGE_CHECK_FISHING_STATS" then
        --print()
        event.text.data = "Статистика о рыбалке: \n" .. 
            "Успешные попытки рыбалки: " .. tostring(math.floor(Hyperspace.playerVariables.fish_caught)) .. "\n" ..
            "Поймано обычной рыбы: " .. tostring(math.floor(Hyperspace.playerVariables.fish_caught_basic)) .. "\n" ..
            "Поймано необычной рыбы: " .. tostring(math.floor(Hyperspace.playerVariables.fish_caught_uncom)) .. "\n" ..
            "Поймано редкой рыбы: " .. tostring(math.floor(Hyperspace.playerVariables.fish_caught_rare)) .. "\n" ..
            "Поймано легендарной рыбы: " .. tostring(math.floor(Hyperspace.playerVariables.fish_caught_leg)) .. "\n" ..
            "Поймано уникальной рыбы: " .. tostring(math.floor(Hyperspace.playerVariables.fish_caught_unique)) .. "\n" ..
            "Поймано рыбы-пушек: " .. tostring(math.floor(Hyperspace.playerVariables.fish_caught_guns)) --.. "\n\n\n" ..
            --"Fishes Consumed:"
    end
    ...
    if string.sub(event.eventName, 0, 23) == "STORAGE_CHECK_FISH_MAW_" and string.sub(event.eventName, stringLeg-7, stringLeg) == "_UPGRADE" then
        event.text.data = "Что вы хотите сделать?\n\nОчки улучшения: " .. tostring(math.floor(Hyperspace.playerVariables.fish_maw_upgrade))
    end
    if event.eventName == "STORAGE_CHECK_FISH_MAW_CONSUME" then
        event.text.data = "Чем вы хотите накормить Мав??\n\nОчки улучшения: " .. tostring(math.floor(Hyperspace.playerVariables.fish_maw_upgrade))
    end
end)
